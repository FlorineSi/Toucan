...
public class LesCases implements Observer{
	...
	public void initCases(int n){
		...
		int positionX = 30;
		Random r = new Random();
		Case casetest = new Case(" ", 30, 50, Color.BLACK);
		...
		for(int i=0; i<n; i++){
			...
			int nb = (int) (0 + Math.random() * (150 - 0 + 1));
			// Création de la case et ajout à l'arraylist
			lesCases.add(new Case(""+nb,(positionX+i*50+i*10),170,Color.MAGENTA));
		}
	}
	...
	public void mouvementVide(int i, int nbFois){
		for(int k = 0; k<lesCases.size()-1;k++){
			if(k!=i)
